...
export function getMathRange( position ) {
	let out ={
		range: null,
		outerRange: null,
		mode : ''
	...
	let findMarkPos=function(position, marks, forward){
		let data={ pos: null,	mark:-1,  markPos:null	};
		let node = getElementAncestor(position);
		if (!node) return data;
		let nodeRange = ViewRange.createIn(node);
	...
	const start = findMarkPos(position, MATHCONFIG.STARTMARKS, false);
	const end   = findMarkPos(position, MATHCONFIG.ENDMARKS,   true);

	if (start.pos !== null && end.pos !== null){
		const closerEnd = findMarkPos(position, MATHCONFIG.ENDMARKS, false);
		if (closerEnd.pos!==null){
			if (closerEnd.pos.isAfter(start.pos)) return out; //not valid
		}
		const closerStart = findMarkPos(position, MATHCONFIG.STARTMARKS, true);
...
export function getMath( position ) {
	let data  = getMathRange(position);
